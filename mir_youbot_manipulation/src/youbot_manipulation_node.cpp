...
ManipulatorRosNode::on_activate(const rclcpp_lifecycle::State& state)
{
  ...
  joint_positions_action_server =
      rclcpp_action::create_server<mir_interfaces::action::MoveToJointAngles>(
          ...
          std::bind(&ManipulatorRosNode::jointAnglesSelectorCancelCallback, this,
                    std::placeholders::_1),
          std::bind(&ManipulatorRosNode::jointAnglesAcceptedCallback, this,
                    std::placeholders::_1));

  cartesian_pose_action_server =
      rclcpp_action::create_server<mir_interfaces::action::CartesianCoordinates>(
          shared_from_this(), "~/cartesian_pose",
          std::bind(&ManipulatorRosNode::cartesianPoseHandleCallback, this,
                    std::placeholders::_1, std::placeholders::_2),
          std::bind(&ManipulatorRosNode::cartesianPoseSelectorCancelCallback, this,
                    std::placeholders::_1),
...
rclcpp_action::CancelResponse ManipulatorRosNode::jointAnglesSelectorCancelCallback(
        ...
        goal_handle)
{
  ...
  auto youbot_angles_set_point =
      youbot_manipulator->convertDoubleToJointAngleSetpoint(joint_positions);
...
rclcpp_action::CancelResponse ManipulatorRosNode::cartesianPoseSelectorCancelCallback(
    const std::shared_ptr<
        ...
        goal_handle)
{
  ...
  target_pose.M = KDL::Rotation::Quaternion(
      cartesian_coordinates.pose.orientation.x, cartesian_coordinates.pose.orientation.y,
      cartesian_coordinates.pose.orientation.z, cartesian_coordinates.pose.orientation.w);
